...
# from lightweight_human_pose.msg import PoseFlame
...
def infer_fast(net, img, net_input_height_size, stride, upsample_ratio, cpu,
    ...
    scale = net_input_height_size / height## Citation:
...
class Flame :
    ...
    def img_cb(self, msg):
    #def run_demo(self,net, image_provider, height_size, cpu, track, smooth):
        checkpoint_path = rospy.get_param( rospy.get_name() + "/checkpoint_path", "checkpoint_iter_370000.pth")
        height_size = rospy.get_param( rospy.get_name() + "/height_size", 256)
        cpu = rospy.get_param( rospy.get_name() + "/cpu", False )
        track = rospy.get_param( rospy.get_name() + "/track", 1 )
        ...
        for pose in current_poses:
            pose.draw(img)
        img = cv2.addWeighted(orig_img, 0.6, img, 0.4, 0)   #画像に重みをつけて合成
        for pose in current_poses:
